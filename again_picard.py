...
def picard_iteration(phi0, lambda_val, K, h, epsilon=1e-5, max_iter=100, c_const = False):
    ...
    cur_sigma  = np.linalg.norm(fm - phi, ord='fro') / np.sqrt(nx*ny)
...
if __name__ == '__main__':
    ...
    for lamb in lambdas:
        for K in Ks:
            smoothed_image, cur_sigma  = picard_iteration(fn, lambda_val=lamb, K=K,h=h,epsilon=1e-5,max_iter=100) # Pass both images
