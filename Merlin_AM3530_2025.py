...
def calculate_diffusion_coefficients(image, K, h=1):
    """Calculates the diffusion coefficients c_east, c_west, c_north, c_south."""
    Ny, Nx = image.shape
    phi = image.astype(float)  # Ensure float type for calculations

    # Apply a mild Gaussian blur to reduce noise in gradient estimates
    phi_blurred = phi.copy()

    dphi_dx = np.zeros_like(phi)
    dphi_dy = np.zeros_like(phi)
    c_east = np.zeros_like(phi)
    c_west = np.zeros_like(phi)
    c_north = np.zeros_like(phi)
    c_south = np.zeros_like(phi)

    # Approximate image gradient (central differences)
    dphi_dx[1:-1, 1:-1] = (phi_blurred[2:, 1:-1] - phi_blurred[:-2, 1:-1]) / (2*h)
    dphi_dy[1:-1, 1:-1] = (phi_blurred[1:-1, 2:] - phi_blurred[1:-1, :-2]) / (2*h)

    # Handle boundaries (using mirroring - IMPROVED)
    dphi_dx[0, :] = (phi_blurred[1, :] - phi_blurred[0, :]) / h
    dphi_dx[Nx-1, :] = (phi_blurred[Nx-1, :] - phi_blurred[Nx-2, :]) / h
    dphi_dy[:, 0] = (phi_blurred[:, 1] - phi_blurred[:, 0]) / h
    dphi_dy[:, Ny-1] = (phi_blurred[:, Ny-1] - phi_blurred[:, Ny-2]) / h

    # Calculate gradient magnitude at cell faces
    grad_mag_east = (dphi_dx[:, :-1] + dphi_dx[:, 1:]) / 2
    grad_mag_west = (dphi_dx[:, :-1] + dphi_dx[:, 1:]) / 2
    grad_mag_north = (dphi_dy[:-1, :] + dphi_dy[1:, :]) / 2
    grad_mag_south = (dphi_dy[:-1, :] + dphi_dy[1:, :]) / 2

    # Diffusion coefficients at cell faces
    c_east[:, :-1] = 1 / (1 + (grad_mag_east / K)**2)
    c_west[:, 1:] = 1 / (1 + (grad_mag_west / K)**2)
    c_north[:-1, :] = 1 / (1 + (grad_mag_north / K)**2)
    c_south[1:, :] = 1 / (1 + (grad_mag_south / K)**2)

    # Handle boundaries (copying values)
    c_east[:, -1] = c_east[:, -2] if Nx > 1 else c_east[:, 0]
    c_west[:, 0] = c_west[:, 1] if Nx > 1 else c_west[:, 0]
    c_north[-1, :] = c_north[-2, :] if Ny > 1 else c_north[0, :]
    c_south[0, :] = c_south[1, :] if Ny > 1 else c_south[0, :]

    #We don't need c anymore
    c = np.zeros_like(phi)

    return c_east, c_west, c_north, c_south, c


def construct_sparse_matrix(Nx, Ny, h, lambda_val, c_east, c_west, c_north, c_south, boundary_condition='neumann'):
    N = Nx * Ny
    row = []
    col = []
    data = []
    ...
    for i in range(Nx):
        for j in range(Ny):
            ...
            # Interior points
            diag_val = (c_east[i,j] + c_west[i,j] + c_north[i,j] + c_south[i,j]) / (h**2) + lambda_val

            #Right Neighbor
            if i < Nx - 1:
              right_neighbor = k + 1
              right_val = -c_east[i,j] / (h**2)
              row.append(k)
              col.append(right_neighbor)
              data.append(right_val)
            else:
              if boundary_condition == 'neumann':
                diag_val += c_east[i,j] / (h**2) #Adjust diagonal for Neumann BC

            #Left Neighbor
            if i > 0:
              left_neighbor = k - 1
              left_val = -c_west[i,j] / (h**2)
              row.append(k)
              col.append(left_neighbor)
              data.append(left_val)
            else:
              if boundary_condition == 'neumann':
                diag_val += c_west[i,j] / (h**2) #Adjust diagonal for Neumann BC

            #Top Neighbor
            if j < Ny - 1:
              top_neighbor = k + Nx
              top_val = -c_north[i,j] / (h**2)
              row.append(k)
              col.append(top_neighbor)
              data.append(top_val)
            else:
              if boundary_condition == 'neumann':
                diag_val += c_north[i,j] / (h**2) #Adjust diagonal for Neumann BC

            #Bottom Neighbor
            if j > 0:
              bottom_neighbor = k - Nx
              bottom_val = -c_south[i,j] / (h**2)
              row.append(k)
              col.append(bottom_neighbor)
              data.append(bottom_val)
            else:
              if boundary_condition == 'neumann':
                diag_val += c_south[i,j] / (h**2)  #Adjust diagonal for Neumann BC
...
def smooth_image_picard(image_noisy, lambda_val, K, epsilon=1e-5, max_iter=100):
    ...
    Args:
        image_noisy (np.array): The input noisy image (2D numpy array).
        lambda_val (float): The fidelity parameter (lambda).
        K (float): The K parameter for anisotropic diffusion.
    ...
    for iter_num in range(max_iter):
        ...
        phi_k = u_k.reshape((Ny, Nx))
        c_east, c_west, c_north, c_south, c = calculate_diffusion_coefficients(phi_k, K, h)

        # 2. Construct the matrix A
        A = construct_sparse_matrix(Nx, Ny, h, lambda_val, c_east, c_west, c_north, c_south)

        # 3. Construct the vector f (use the ORIGINAL image)
        ...
        print(f"Iteration {iter_num+1}: Error = {error}")
        if error < epsilon:
